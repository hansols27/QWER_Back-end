...
    try {
        // 클라이언트에서 받은 데이터를 기본 타입으로 캐스팅
        const rawData = req.body as Omit<ScheduleEvent, 'id' | 'color'>;
        
        // ⭐️ 추가: 문자열로 넘어온 start/end 필드를 Date 객체로 변환
        // (FullCalendar 등은 ISO 문자열을 보내므로, Date 객체로 변환해야 서비스에서 처리 가능)
        const data = {
            ...rawData,
            start: new Date(rawData.start as unknown as string),
            end: new Date(rawData.end as unknown as string),
        } as Omit<ScheduleEvent, 'id' | 'color'>;

        // 유효성 검사 (Date 객체로 변환 실패 시)
        if (isNaN(data.start.getTime()) || isNaN(data.end.getTime())) {
            return res.status(400).json({ success: false, error: '시작일 또는 종료일 형식이 잘못되었습니다.' });
        }
        
...
    try {
        const id = req.params.id;
        // Partial<Omit<ScheduleEvent, 'id'>>는 color를 수정할 수 있도록 허용합니다.
        const rawData = req.body as Partial<Omit<ScheduleEvent, 'id'>>; 
        
        const data: Partial<Omit<ScheduleEvent, 'id'>> = { ...rawData };
        
        // ⭐️ 추가: start/end 필드가 있다면 Date 객체로 변환
        if (data.start && typeof data.start === 'string') {
            const newDate = new Date(data.start);
            if (isNaN(newDate.getTime())) {
                return res.status(400).json({ success: false, error: '시작일 형식이 잘못되었습니다.' });
            }
            data.start = newDate;
        }
        if (data.end && typeof data.end === 'string') {
            const newDate = new Date(data.end);
            if (isNaN(newDate.getTime())) {
                return res.status(400).json({ success: false, error: '종료일 형식이 잘못되었습니다.' });
            }
            data.end = newDate;
        }
        
